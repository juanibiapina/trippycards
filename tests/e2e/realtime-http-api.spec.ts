...
function connectWebSocket(roomId: string) {
  return new Promise<{ ws: WebSocket, nextMessage: () => Promise<unknown> }>((resolve, reject) => {
...
async function waitForMessageOfType(nextMessage: () => Promise<unknown>, type: string, maxTries = 5) {
  for (let i = 0; i < maxTries; ++i) {
    const msg = await nextMessage();
    if ((msg as { type?: string }).type === type) return msg;
  }
