...
import { LinkCard, LinkCardInput, PollCardInput, AILinkCardInput } from '../../../shared';
...
interface CardCreationModalProps {
  ...
  onClose: () => void;
  onCreateCard: (
    card: LinkCardInput | PollCardInput | AILinkCardInput
  ) => void;
...
export const CardCreationModal: React.FC<CardCreationModalProps> = ({
  ...
  const [cardType, setCardType] = useState<'link' | 'poll' | 'ailink'>('link');
        ...
        <div className="p-6">
          ...
          {cardType === 'ailink' && (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end space-x-2 mt-6">
                <button type="button" onClick={handleClose} className="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300">Cancel</button>
                <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Create AILink Card</button>
              </div>
            </form>
          )}
        </div>
